...
Swift 版采用原生 `AsonValue` 值模型，而不是反射或外部 schema 描述符。
...
- Swift 侧的数据通过 `AsonValue` 表达。
- 文本解码读取文本里内嵌的 schema，并返回 `AsonValue`。
- 当前 binary API 会携带足够的 typed 信息，因此可以直接 round-trip 回 `AsonValue`，不需要额外 schema 字符串。
...
- Swift 当前主要工作在动态 `AsonValue` 模型上，而不是宿主类型反射。
