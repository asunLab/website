...
| ASON 类型 | Rust | Go | Python | Java / Kotlin | Swift | C | C++ | Zig | C# | Dart | JS / TS | PHP |
|-----------|------|----|--------|---------------|-------|---|-----|-----|----|------|---------|-----|
...
| 嵌套结构体 | `struct` | `struct` | `dict` / 工厂返回对象 | class / data class | `AsonValue.object` / 宿主模型 | `struct` + schema 描述符 | `struct` + 元数据宏 | `struct` | class / record + schema 接口 | 实现 `AsonSchema` 的类 | 普通对象 | 关联数组 / 对象式数组 |
...
| Swift | Swift tools `5.9+` | 围绕 `AsonValue` 的原生 Swift 值模型 | 文本自带 schema，返回 `AsonValue` | binary 自带足够 typed 信息，返回 `AsonValue` |
...
- 二进制 ASON 刻意不做自描述，因此大多数实现都需要额外提供：
  ...
  - schema 描述符，
  - 或 schema 字符串。
- 当前仓库里的 Swift 是主要例外：它的 binary API 会携带足够的 typed 信息，因此可以直接 round-trip 成 `AsonValue`。
